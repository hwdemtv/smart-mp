...
[![Version](https://img.shields.io/badge/version-1.6.0-blue.svg)](https://github.com/hwdemtv/smart-mp)
...
- **直连/代理可选**: 默认直连微信官方 API，Pro 用户可选使用 **中心令牌服务器** 以绕过 IP 白名单限制。
...
### 场景 B：AI 辅助创作
> **目标**: 快速润色文案并生成标题。
1. **润色**: 选中段落 → `Ctrl+Alt+L`（或在设置中自定义快捷键）→ AI 润色，通过流式对话框实时优化文笔。
2. **起名**: 运行 `mp-headline` 命令，AI 自动根据全文生成 10 个爆款标题供选择。
3. **配图**: 使用文生图功能为文章自动生成封面图。

### ⌨️ 默认快捷键（均可在 Obsidian 快捷键设置中修改）

| 快捷键 | 功能 |
| :--- | :--- |
| `Ctrl/Cmd + Alt + P` | 打开文章渲染预览 |
| `Ctrl/Cmd + Alt + L` | AI 润色选中文本 |
| `Ctrl/Cmd + Alt + C` | 复制文章到剪贴板（富文本） |
| `Ctrl/Cmd + Alt + S` | 发送文章到微信草稿箱 |
| `Ctrl/Cmd + Alt + E` | 导出文章为 HTML 文件 |
...
**痛点**: 用户在移动办公时 IP 变动频繁，导致微信 API 调用失败，需频繁手动修改白名单。
**解决方案 (SmartMP Pro)**: 提供稳定的 **中心令牌服务器**，彻底解决动态 IP 问题，无需任何网络配置，即插即用。

---

## 💎 定价方案 (Pricing)

SmartMP 采用 **"本地功能 + 云端服务"** 的模式，兼顾实惠与高效。

### 1. SmartMP Pro
> **¥69/年** (订阅制)

适合专业创作者与多设备用户，解锁云端增强功能。
- ✨ **中心令牌服务器**: 无需公网 IP 即可同步，彻底解决动态 IP 白名单问题。
- 🛠️ **优先支持**: 一对一解决使用问题。
- 📦 **永久更新**: 包含所有未来本地新功能。
- 🎁 **多端漫游**: 支持最多 3 台个人设备自动漫游验证。
...
- **微信 API 调用**: 当您使用"发布草稿"功能时，插件会直接与微信公众平台 API (`api.weixin.qq.com`) 或您选择的 **中心令牌服务器** 通信，用于上传素材和发布文章。
