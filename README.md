...
- **高精度滚动同步**: 采用基于标题锚点 (Heading-Anchor) 的同步算法，适配图片较多的长文章。
- **固定头部布局**: 预览界面的标题和属性面板固定在顶部，方便随时查看。
- **快捷键支持**: 支持通过 `Toggle Scroll Sync` 命令绑定快捷键切换同步状态。
- **样式细节优化**: 修正了部分代码块和预览区域的边边角角样式。

---

WeWrite 是一款将 Obsidian 的强大功能与微信公众号编辑需求完美契合的插件。它使您能够通过简单的操作，将精心撰写的 Markdown 笔记一键转化为精美的微信公众号文章。平台，同时集成 AI 写作辅助功能，使 Obsidian 成为微信公众号内容的强大写作工具。不依赖外部的图床或第三方平台。
