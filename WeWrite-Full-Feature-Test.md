...
> [!abstract] 简介
> 本文档包含了 WeWrite 插件 README 中提到的所有 Obsidian 特色渲染功能。您可以将其放入 Obsidian 库中，通过 WeWrite 预览面板查看在微信公众号环境下的渲染效果。

---

## 1. 基础 Markdown 渲染
这里测试基础格式：**加粗**、*斜体*、~~删除线~~、`行内代码`、以及[外部链接](https://github.com/LearnerChen/wewrite)。
...
const wewrite: WeWrite = {
    cool: true,
    support: ["Mermaid", "LaTeX", "Excalidraw"]
};

console.log("WeWrite 让排版变得简单！");
```
...
> [!info] 这是一个信息提示
> 微信公众号原生不支持这种卡片，WeWrite 将其转换为带图标的漂亮 Section。
...
[^1]: 此脚注在 WeWrite 预览中会变成数字上标，并在文末生成“参考链接”列表。
...
> [!note] 提示
> 此处演示的是 `![[]]` 语法，请确保您的库中存在对应的文件名，WeWrite 会将其递归渲染。

---

## 11. PDF 裁剪 (需 PDF++)
![[document.pdf#page=1&rect=10,20,100,200]]
> WeWrite 会尝试抓取 PDF++ 插件渲染出的裁剪区域。
