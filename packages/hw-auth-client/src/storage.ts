...
export class AuthStorage {
   ...
   * 读取全局配置
   */
  async read(): Promise<GlobalAuthConfig | null> {
    try {
      ...
      const config = JSON.parse(content) as GlobalAuthConfig;
   ...
   * 写入全局配置
   */
  async write(config: GlobalAuthConfig): Promise<boolean> {
    try {
   ...
   * 获取指定产品的激活记录
   */
  async getActivation(productId: string): Promise<Activation | null> {
   ...
   * 更新指定产品的激活记录
   */
  async updateActivation(productId: string, activation: Activation): Promise<boolean> {
   ...
   * 移除指定产品的激活记录
   */
  async removeActivation(productId: string): Promise<boolean> {
    const config = await this.read();
   ...
   * 检查是否有任一产品已激活
   */
  async hasAnyActivation(): Promise<boolean> {
    const config = await this.read();
    if (!config) return false;
   ...
   */
  async getSharedLicenseKey(): Promise<string | null> {
    const config = await this.read();
    if (!config) return null;
