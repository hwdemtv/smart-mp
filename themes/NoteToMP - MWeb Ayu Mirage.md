---
theme_name: NoteToMP MWeb Ayu Mirage
---

```css
/*
 * PrismJS default theme
 * 这个文件只应该包含变量
 */
/*
 * Default bear theme，适用于 core/mweb-bear.scss。
 * 所有 bear 主题，只需 import 这个文件，然后修改配色方案即可。
 */
/* font */
/* container */
/*  spacing */
/* color */
/* other */
/**
 * 在 bear 的主题中，某些变量的取值和其他变量是绑定的。
 * 统一写在这里，这个文件应该在变量文件的最后被引入。
 */
/**
 * Bear 的默认样式表。通过调整各个颜色变量的取值，就可以得到不同的 bear 主题。
 * Bear 的配色方案位于 src/themes/bear-palettes 目录下。
 */
.smart-mp {
  font-size: 16px;
  color: rgb(217, 215, 209);
  background-color: rgb(31, 35, 47);
  line-height: 1.6em;
  margin: 0 0;
  padding: 1em 1em;
}
.smart-mp p,
.smart-mp pre,
.smart-mp dl,
.smart-mp form,
.smart-mp details,
.smart-mp dl,
.smart-mp blockquote,
.smart-mp table,
.smart-mp xmp,
.smart-mp plaintext,
.smart-mp listing,
.smart-mp figure {
  margin: 0.75em 0 0.45em;
}
.smart-mp hr {
  margin: 0.75em auto;
}
.smart-mp h1,
.smart-mp h2,
.smart-mp h3,
.smart-mp h4,
.smart-mp h5,
.smart-mp h6 {
  margin-top: 1.5em;
  margin-bottom: 0.75em;
  margin-left: 0;
  margin-right: 0;
  font-weight: 600;
  line-height: 1.5em;
  color: rgb(159, 170, 185);
}
.smart-mp h1 {
  font-size: 1.5em;
}
.smart-mp h2 {
  font-size: 1.3em;
}
.smart-mp h3 {
  font-size: 1.1em;
}
.smart-mp h4 {
  font-size: 1em;
}
.smart-mp h5 {
  font-size: 1em;
}
.smart-mp h6 {
  font-size: 1em;
}
.smart-mp hr {
  height: 1px;
  border: 0;
  background-color: rgb(60, 66, 84);
  border-style: inset;
  border-width: 1px;
}
.smart-mp p {
  margin-left: 0;
  margin-right: 0;
}
.smart-mp pre {
  padding: 0;
  border: 0;
}
:root {
  --blockquote-background-color: #181b25;
}

.smart-mp blockquote {
  display: block;
  padding-left: 0.8em;
  border-left: 0.2em solid rgb(254, 203, 102);
  color: rgb(217, 215, 209);
  background-color: var(--blockquote-background-color, #181b25);
}
.smart-mp blockquote > :first-child {
  margin-top: 0;
}
.smart-mp blockquote > :last-child {
  margin-bottom: 0;
}
.smart-mp li {
  word-wrap: break-all;
}
.smart-mp ul {
  margin-left: 1.3em;
  padding: 0;
}
.smart-mp li::marker {
  color: rgb(254, 203, 102);
}
.smart-mp li > p {
  margin: 0;
}
.smart-mp ol {
  padding-left: 1.3em;
  list-style-type: decimal;
}
.smart-mp img {
  max-width: 100%;
  height: auto;
}
.smart-mp u {
  text-decoration: none;
  background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0) 50%, rgb(254, 203, 102) 50%);
  background-repeat: repeat-x;
  background-size: 2px 2px;
  background-position: 0 1em;
}
.smart-mp a {
  color: rgb(254, 203, 102);
  text-decoration: none;
}
.smart-mp a img {
  border: none;
}
.smart-mp b,
.smart-mp strong {
  font-weight: bold;
}
.smart-mp i,
.smart-mp cite,
.smart-mp em,
.smart-mp var,
.smart-mp address,
.smart-mp dfn {
  font-style: italic;
}
.smart-mp del,
.smart-mp s {
  color: rgb(120, 126, 140);
}
.smart-mp pre,
.smart-mp xmp,
.smart-mp plaintext,
.smart-mp listing,
.smart-mp code,
.smart-mp kbd,
.smart-mp tt,
.smart-mp samp {
  font-family: Menlo-Regular, Menlo, Monaco, Consolas, "Courier New", monospace;
}
.smart-mp mark {
  color: inherit;
  display: inline;
  padding: 0.2em 0.5em;
  background-color: rgb(17, 110, 84);
}
.smart-mp figcaption {
  text-align: center;
}
.smart-mp table {
  color: rgb(207, 208, 203);
  border-collapse: collapse;
  background-color: rgb(24, 28, 37);
  border-spacing: 2px;
  font-size: 1em;
  border: 1px;
  border-spacing: 0;
}
.smart-mp th,
.smart-mp td {
  padding: 0.7em 1em;
  font-size: 0.9em;
  border: 1px solid rgb(60, 66, 84);
}
.smart-mp caption,
.smart-mp th,
.smart-mp td {
  text-align: left;
  font-weight: normal;
  vertical-align: middle;
}
.smart-mp .footnotes > ol li {
  text-indent: 0;
}
.smart-mp .footnotes hr {
  margin-top: 4em;
  margin-bottom: 0.5em;
}
.smart-mp code {
  display: inline;
  color: rgb(207, 208, 203);
}

/* 代码块 */
.smart-mp .code-section {
  display: flex;
  border: solid 1px rgb(60, 66, 84);
  margin: 1.5em 0;
  line-height: 26px;
  padding: 0.5em;
  font-family: Menlo-Regular, Menlo, Monaco, Consolas, "Courier New", monospace;
}
.smart-mp .code-section pre {
  margin: 0;
  margin-block-start: 0;
  margin-block-end: 0;
  white-space: normal;
  overflow-x: auto;
  padding: 0 0 0 1em;
}
.smart-mp .code-section code {
  display: flex;
  text-wrap: nowrap;
  font-family: Menlo-Regular, Menlo, Monaco, Consolas, "Courier New", monospace;
}
.smart-mp .code-section ul {
  margin: 0;
  padding: 0;
  margin-block-start: 0;
  margin-block-end: 0;
  width: fit-content;
  flex-shrink: 0;
  height: 100%;
  line-height: 26px;
  list-style-type: none;
}
.smart-mp .code-section ul > li {
  text-align: right;
}

/* 高亮文本样式 */
.smart-mp mark, .smart-mp .highlight {
  background-color: #ffeb3b !important;
  color: #333 !important;
  padding: 0 2px !important;
  border-radius: 3px !important;
}

/* 分隔符样式 */
.smart-mp-hr-replacement {
  margin: 30px 0 !important;
  height: 2px !important;
  background: linear-gradient(to right, transparent, rgba(255, 106, 0, 0.5), transparent) !important;
  font-size: 0 !important;
  border: none !important;
}
```
